...
const ScrollContainer = ({ children }: ScrollContainerProps) => {
  ...
  const { getHistoryMessagesPageState, messageCache, slug } =
    useRootStore().messageStore;
  const {
    isOpenHashTagScreen,
    getHistoryHashTagsMessagesPageState,
    allHashTagsMessages,
  ...
  useEffect(() => {
    ...
    if (innerDivHeight < outerDivHeight) {
      ...
      if (prevInnerDiv.current) {
        ...
        console.log(
          "top1",
          innerDivHeight - prevInnerDiv.current + outerDivScrollTop,
        );
  ...
  const [isFetching, setIsFetching, stop] = useInfiniteScroll(
    getMoreMessages,
    outerDiv,
  );
