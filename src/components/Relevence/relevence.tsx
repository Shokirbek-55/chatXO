...
const Relevence = () => {
    ...
    const { memberRelevence, updateMemberRelevance, setRelevanceChange, relevanceData } = useRootStore().channelStore;
    ...
    return (
        <div className={styles.container} style={{ display: visible.RelevenceModal ? 'block' : 'none' }}>
            <div className={styles.box}>
                <RowItemView
                    ...
                    imageUrl={memberRelevence?.avatar ? `${TMP_URL}/${memberRelevence?.avatar}` : ''}
                ...
                <div className={styles.btnBox}>
                    <ImCancelCircle onClick={() => hide('RelevenceModal')} className={styles.cancel} size={24} />
                    <BiCheckCircle onClick={updateRelevance} className={styles.choose} size={28} />
                </div>
            </div>
        </div>
    );
