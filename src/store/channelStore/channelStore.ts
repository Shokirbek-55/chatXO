...
export default class ChannelStore {
  ...
  createChannelOperation = new Operation<createChannelResponseType>(
    {} as createChannelResponseType,
  );
  ...
  get getSelectedChannelData() {
    return (
      this.channelByHashId.get(this.selectedChannelData.hashId) ||
      ChannelInitialState
    );
  }
  ...
  channelsSort = () => {
    this.myChannels = this.myChannels.sort((a, b) => {
      return (
        Number(new Date(b.lastMessageTimestamp)) -
        Number(new Date(a.lastMessageTimestamp))
      );
    });
  };
      ...
      await Promise.all(
        this.myChannels.map(async (channel) => {
          ...
          const [channelUsersData, channelData] = await Promise.all([
            ...
            this.getChannelByHashIdOperation.run(() =>
    ...
    runInAction(() => {
      if (this.createChannelAvatarOperation.isSuccess) {
        this.setUpdateChannelState(
          "avatar",
          Object.values(this.createChannelAvatarOperation.data).toString(),
        );
        this.channelAvatar = "";
        this.chFormData = new FormData();
