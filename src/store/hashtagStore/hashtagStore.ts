...
export default class HashtagStore {
    ...
    setAllHashTagsMessages = (data: { messages: RawMessage[]; pageState: string; end: boolean }) => {
        if (!!this.allHashTagsMessages.messages) {
            if (data.pageState !== null && this.allHashTagsMessages.pageState === data.pageState) {
                return;
            } else {
                this.allHashTagsMessages.messages = [
                    ...(this.allHashTagsMessages.messages.filter(item => item.id === data.messages[0].id).length === 1
                        ? []
                        : data.messages),
                    ...this.allHashTagsMessages.messages,
                ];
    ...
    getChannelAllHashTags = async () => {
        ...
        await this.getChannelAllHashTagsOperation.run(() => APIs.channels.getChannelAllHashtags(this.hashId));
