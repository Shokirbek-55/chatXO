...
import { RegisterData, Session } from '../../types/auth';
...
export default class AuthStore {
    ...
    verifyOperation = new Operation<Session>({} as Session);
    ...
    deleteUserOperation = new Operation<{ userId: number }>({} as { userId: number });
    resetPassOperation = new Operation<{ email: string }>({} as { email: string });
    ...
    loginOAuth2 = async (data: LoginOAuth2Payload) => {
        ...
        if (this.loginOAuthOperation.data && this.loginOAuthOperation.isSuccess) {
    ...
    register = async (data: any, afterSuccess?: () => void) => {
    ...
    verify = async (
        ...
        if (this.verifyOperation.data && this.verifyOperation.isSuccess) {
            this.root.localStore.setToken(this.verifyOperation.data.data);
            this.root.runFunctionsWithLogin();
        }
        if (this.verifyOperation.isError) {
            message.error(`${this.verifyOperation.error}`);
        }
    };
