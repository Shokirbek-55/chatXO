...
export default class MessageStore {
		...
		if (!!this.messageCache[slug]) {
			if (this.messageCache[slug]?.pageState === pageState) {
				...
				this.messageCache[slug].messages = [
					...(this.messageCache[slug].messages.filter((item) => item.id === messages[0]?.id)
						.length === 1
						? []
						: [...messages]),
					...this.messageCache[slug].messages,
	...
	onSendMessage = (type?: MessageType) => {
		...
		if (this.editMessage) {
			const editedMessage = { ...this.editMessage, message: this.messageTextState };
	...
	onDeleteMessage = (id: string) => {
		runInAction(() => {
			this.messageCache[this.slug].messages = this.messageCache[this.slug].messages.filter(
				(e) => e.id !== id
			);
		});
	};
