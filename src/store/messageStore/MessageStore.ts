...
export default class MessageStore {
  ...
  get getSelectedChannelMsgData() {
    return this.channelsMsgDataBySlug.get(
      this.app.channelStore.selectedChannelData.slug,
    );
  }

  get getSelectedChannelMsgsMap() {
    return (
      this.getSelectedChannelMsgData?.messages || new Map<string, RawMessage>()
    );
  }
  ...
  removePollOption = (v: string, index: number) => {
    runInAction(() => {
      if (this.pollMessageOptionState.length > 2)
        this.pollMessageOptionState = this.pollMessageOptionState.filter(
          (e, i) => i !== index,
        );
    });
  };
